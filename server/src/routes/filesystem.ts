...
async function browseDirectory(dirPath: string): Promise<BrowseResult> {
  ...
  // Sort: .squad folders first, then alphabetical
  entries.sort((a, b) => {
...
router.get('/browse', async (req, res) => {
  try {
    ...
    if (!requestedPath) {
      result = await listDrives();
    } else {
      result = await browseDirectory(requestedPath);
    }

    res.json(result);
  } catch (err: any) {
    console.error('Filesystem browse error:', err.message);
